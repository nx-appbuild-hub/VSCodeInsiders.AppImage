...
all: clean
	...
	wget --output-document=$(PWD)/build/build.rpm https://packages.microsoft.com/yumrepos/vscode/code-insiders-1.63.0-1637828377.el7.x86_64.rpm
	...
	echo '' 																		>> $(PWD)/build/Boilerplate.AppDir/AppRun
	echo '' 																		>> $(PWD)/build/Boilerplate.AppDir/AppRun
	echo 'exec $${APPDIR}/vscode/code-insiders $${@}' 						        >> $(PWD)/build/Boilerplate.AppDir/AppRun
	...
	export ARCH=x86_64 && $(PWD)/bin/appimagetool.AppImage $(PWD)/build/Boilerplate.AppDir $(PWD)/VSCodeInsiders.AppImage
	chmod +x $(PWD)/VSCodeInsiders.AppImage
